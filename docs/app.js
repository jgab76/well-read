...
let books=[],category='All books',query='';
function categories(){return [...new Set(books.flatMap(b=>b.categories))].sort((a,b)=>a.localeCompare(b))}
function cover(b){return b.cover || (b.isbn?`https://covers.openlibrary.org/b/isbn/${encodeURIComponent(b.isbn)}-M.jpg?default=false`:(/^\d+$/.test(b.id)?`https://covers.openlibrary.org/b/goodreads/${b.id}-M.jpg?default=false`:''))}
...
function render(){
 ...
 $('#shelves').querySelectorAll('img').forEach(img=>img.addEventListener('error',()=>{const b=books.find(b=>b.id===img.dataset.coverId);img.parentElement.innerHTML=fallback(b)}));
}
...
$('#shelves').onclick=e=>{
 const button=e.target.closest('[data-id]');if(!button)return;const b=books.find(b=>b.id===button.dataset.id);
 ...
 $('#detail img')?.addEventListener('error',e=>e.target.parentElement.remove());$('#detail-dialog').showModal();
};
